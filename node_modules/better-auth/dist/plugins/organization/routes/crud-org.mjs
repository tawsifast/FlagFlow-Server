...
const getOrganizationQuerySchema = z.optional(z.object({
...
const getOrganization = (options) => createAuthEndpoint("/organization/get-organization", {
	method: "GET",
	...
	const adapter = getOrgAdapter(ctx.context, options);
	const organization = ctx.query?.organizationSlug ? await adapter.findOrganizationBySlug(organizationId) : await adapter.findOrganizationById(organizationId);
...
//#endregion
export { checkOrganizationSlug, createOrganization, deleteOrganization, getFullOrganization, getOrganization, listOrganizations, setActiveOrganization, updateOrganization };
