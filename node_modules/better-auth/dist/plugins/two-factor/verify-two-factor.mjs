		...
		return {
			...
			beginAttempt: async (allowedAttempts) => {
				...
				if (attempts >= allowedAttempts) {
					try {
						await ctx.context.internalAdapter.consumeVerificationValue(signedTwoFactorCookie);
					} catch {
						expireCookie(ctx, twoFactorCookie);
...
async function recordTwoFactorFailure(ctx, twoFactorTable, twoFactor) {
	...
	}))?.failedVerificationCount ?? 0) >= maxFailedAttempts) await ctx.context.adapter.incrementOne({
		model: twoFactorTable,
		where: [{
			field: "id",
			value: twoFactor.id
